...
# === Log-Mel Pitch RMS-Energy Extraction ===
def LogPitchRMS(file_path, target_duration=4.0, sr=16000, n_mels=40, n_fft=400, hop_length=160):
    y, _ = librosa.load(file_path, sr=sr)

    # 1. Süre sabitleme (2 saniye = 32000 örnek)
    ...
    # 4. RMS Energy
    rms = librosa.feature.rms(y=y, hop_length=hop_length)  # (1, T)

    # 5. Zaman boyutunu log-mel'e eşitle
    T = log_mel.shape[1]
    def match_time(x):
        if x.shape[1] > T:
            return x[:, :T]
        elif x.shape[1] < T:
            return np.pad(x, ((0, 0), (0, T - x.shape[1])), mode='edge')
        return x

    f0 = match_time(f0)
    rms = match_time(rms)

    # 6. Pitch ve RMS’i yukarı doğru kopyala (tile): (40, T)
    f0_rep = np.tile(f0, (n_mels, 1))
    rms_rep = np.tile(rms, (n_mels, 1))

    # 7. Stack → (40, T, 3) → CNN'e uygun
    stacked = np.stack([log_mel, f0_rep, rms_rep], axis=-1)

    return stacked.astype(np.float32)  # shape: (40, 201, 3)
...
def run_model(wav_list, mode="Pred", model_path_emotion="trained_model_emotion.keras", model_path_name="trained_model_name.keras"):
    ...
    if mode == "train":
        ...
        for filename in wav_list:
            ...
            features = LogPitchRMS(path)  # ← yeni fonksiyon
            X.append(features)
        ...
        print("CNN models for name and emotion recognition trained with LogPitchRMS.")
        ...
        for filename in wav_list:
            ...
            features = LogPitchRMS(path)
            features = np.asarray(features, dtype=np.float32).reshape(1, 40, 401, 3)

            pred_emotion = model_emotion.predict(features)
            pred_name = model_name.predict(features)
...
def evaluate_model_stats(folder_path="Dataset", repeat=10,split_ratio=0.2):
    ...
    for i in range(repeat):
        ...
        train_list, test_list = group_split(folder_path,test_ratio=0.2)
